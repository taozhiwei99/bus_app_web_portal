...
import TeacherChildCsvExample from '../assets/csv-examples/teacherchildcsv.png'

export default function UploadTeacherChildAssignment() {
  // PREVIEW START //
  ...
  const handleSubmit = async () => {
    if (data.length == 0) {
      ...
      const expectedHeader = ['teacherid', 'childid'];
      ...
      axios.post('https://lagj9paot7.execute-api.ap-southeast-1.amazonaws.com/dev/api/schadm-uploadteacherchildassignmentvalidation', {
        data: data,
        si: schoolid
      })
      .then(firstCheckRes => {
        if (firstCheckRes.data.success) {
          axios.post('https://lagj9paot7.execute-api.ap-southeast-1.amazonaws.com/dev/api/schadm-uploadteacherchildassignment', {
            data: data,
          })
          .then(uploadRes=> {
            if (uploadRes.data.success){ alert(uploadRes.data.log) }
            else { alert(uploadRes.data.errlog) }
          })
        } else {
    ...
    <>
      ...
      <CContainer className='px-4 py-2 pb-5'>
        ...
        <CardHeader color="blue-gray" className="pb-2">
          <img
            src={TeacherChildCsvExample}
            alt="Teacher-Child CSV sample image"
          />
        </CardHeader>
        ...
        <div className='pt-5'>
          <Card className="h-full w-full overflow-scroll">
            <table className="w-full min-w-max table-auto text-left">
              <tbody>
                {data.map(({ teacherid, childid }, index) => (
